...
static function getEventTime() {
	...
	return uiGridGrabber.visible ? getTimeFromWorldY(uiGridGrabber.y) : getTimeFromWorldY(GRID_SIZE * 8);
}
...
static var platformHashes = [];
static var editBoneHashIndex:Int = -1;
static var editPlatformHashIndex:Int = -1;

// BULLSHIT FOR PLATFORMS AND BONES //
var hashList = ["bone" => [], "platform" => []];
var hashPendingEdit = ["a" => 0];
hashPendingEdit.remove("a");
// BULLSHIT FOR PLATFORMS AND BONES //
...
function addEvent(event) {
	...
	switch (event.type) {
		case EVENT_BONE:
			...
			bonesHashes.push(event.params[0].id);
...
function editEvent(newData, eventIndex, specificIndex, hashIndex) {
	...
	if (editBoneHashIndex != -1) {
		bonesHashes[hashIndex] = newData.params[0].id;
	}
}
...
function updateGrids(elapsed:Float) {
	if (FlxG.keys.pressed.CONTROL && FlxG.mouse.wheel != 0) {
		gridScale += FlxG.mouse.wheel * 0.1;
	}
	...
	for (i in 0...uiGrids.length) {
		...
		uiGrids[i].y = FlxMath.lerp(uiGrids[i].y, uiGridPosition, 0.5 * 60 * elapsed);
	}
	uiEventBackdrop.y = FlxMath.lerp(uiEventBackdrop.y, uiGridPosition, 0.5 * 60 * elapsed);
	...
	if (!uiEventSelecting)
		uiGridGrabber.x = uiGrids[uiCurrentGridID].x + GRID_SIZE * .25;
	...
	for (i => event in events) {
		...
		var rowIndex = switch (event.type) {
			...
			case EVENT_EDIT_SOUL: 4;
			case EVENT_PLATFORM: 5;
			default: 0;
		};
...
function updateSelection(elapsed) {
	...
	for (item in eventSprites) {
		item.color = item.selected ? 0xFF9C7BE9 : 0xFFFFFFFF;
		item.scaleOff = item.selected ? (slBoxState == B_DRAGGING ? 1.35 : 1.085) : 1;
	}

	uiGridGrabber.visible = slBoxState == B_NONE || slBoxState == B_SELECTED;
...
function clearSelection() {
	for (item in slItems)
		item.selected = false;
	slItems = FlxArrayUtil.clearArray(slItems, false);
	slBoxState = B_NONE;
}
...
function setupUI() {
	...
	uiSongSlider.valueStepper.visible = uiSongSlider.active = false;
			...
			childs: [
				...
				},
				{
					...
					onSelect: (t) -> {
						...
						_ui_delete_event(item.data, false);
...
function _ui_delete_events(events) {
	for (event in events) {
		_ui_delete_event(event, false);
	}
}

function _ui_delete_event(event, ?reversible = true) {
	switch (event.type) {
		case EVENT_BONE:
