...
function postCreate() {
	...
	subHeader = new UIText(30, 110, FlxG.width, 'Initial Data (x,y,angle)              Time    Bone ID', 18);
	add(subHeader);
	...
	subParams = new UIText(30, 250 + 60 + 40, FlxG.width, 'Velocity (x,y,ang)                    Mode', 18);
	add(subParams);

	// bone mode
	final list = ['normal', 'orange', 'blue'];
	...
	visibleOut = new UICheckbox(30, winHeight - 40 - 10, 'Visible outside box', params.out);
	add(visibleOut);
	...
	saveButton = new UIButton(winWidth - 200 - 10, winHeight - 80 - 10, 'Add', () -> {
		...
		} else if ((editingEvent && firstId != idBox.label.text && checkForID(EVENT_BONE, idBox.label.text))
			|| (!editingEvent && checkForID(EVENT_BONE, idBox.label.text))) {
			openSubState(new UIWarningSubstate("Error", "This Bone ID is already registered !", [{label: "Ok", color: 0xFF727272, onClick: function(t) {}}]));
		} else {
			windowOutput = {
				row: getRow(),
				type: EVENT_BONE,
